...
export function Slide(itemList, target, transition = null) {
  const _t = parseInt(getComputedStyle(itemList[0]).transitionDuration.replace("s", ""));
 ...
 */
export function Focus(itemList, target, sideQuantity = 1, transition = null) {
  const _t = parseInt(getComputedStyle(itemList[0]).transitionDuration.replace("s", ""));
  transition = transition || _t ? _t : 0.5;

  const len = itemList.length;
  const targetWidth = target.clientWidth;
  const itemWidth = itemList[0].offsetWidth;
  let i = 0;
  ...
  function zoomIn(i) {
    const offset = i * -100;
    const place = offset + ((targetWidth / itemWidth - 1) / 2) * 100;
    itemList[i].style.transform = `translateX(${place}%) scale(1)`;
  }
  function zoomOut(i, isLeft = true) {
    const shrink = 0.6;
    const offset = i * -100 - (1 - shrink) * 50;
    const place = offset + (targetWidth / itemWidth - 1) * 100 + (1 - shrink) * 100;
    itemList[i].style.transform = `translateX(${isLeft ? offset : place}%) scale(${shrink})`;

    // 屬性scale寫法
    // const offset = i * -100 * (1 / shrink) - ((1 - shrink) / shrink) * 50;
    // const place = offset + (targetWidth / itemWidth - 1) * 100 * (1 / shrink) + ((1 - shrink) / shrink) * 100;
    // itemList[i].style.scale = shrink;
  }
